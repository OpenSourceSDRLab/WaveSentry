...
void CommandLine::RunSetup() {
  ...
  Serial.println(F("\n\n--------------------------------\n"));
  Serial.println(F("         WaveSentry   \n"));
  Serial.println("            " + version_number + "\n");
  Serial.println(F("       By: Community \n"));
  Serial.println(F("--------------------------------\n\n"));
...
void CommandLine::runCommand(String input) {
  ...
  if(wifi_ble_scan_obj.scanning() && wifi_ble_scan_obj.currentScanMode == WIFI_SCAN_GPS_NMEA){
  ...
  if (cmd_args.get(0) == STOPSCAN_CMD) {
    ...
    uint8_t old_scan_mode=wifi_ble_scan_obj.currentScanMode;
    ...
    // If we don't do this, the text and button coordinates will be off
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        wifi_ble_scan_obj.StartScan(WIFI_SCAN_GPS_NMEA, TFT_CYAN);
      }
    #endif
  }
  // LED command
  ...
  // Channel command
    ...
    if (cmd_args.size() == 1) {
      Serial.println("Current channel: " + (String)wifi_ble_scan_obj.set_channel);
    }
    else if (ch_set != -1) {
      wifi_ble_scan_obj.set_channel = cmd_args.get(ch_set + 1).toInt();
      wifi_ble_scan_obj.changeChannel();
      Serial.println("Set channel: " + (String)wifi_ble_scan_obj.set_channel);
    }
  }
  // Clear APs
    ...
    if (ss_sw != -1) {
      ...
      #endif
      wifi_ble_scan_obj.RunClearSSIDs();
    }
    ...
    if (cl_sw != -1) {
        ...
        menu_function_obj.changeMenu(&menu_function_obj.clearAPsMenu);
      #endif
      wifi_ble_scan_obj.RunClearStations();
    }
  }
  ...
  //// WiFi/Bluetooth Scan commands
  if (!wifi_ble_scan_obj.scanning()) {
    // Dump pcap/log to serial too, valid for all scan/attack commands
    wifi_ble_scan_obj.save_serial = this->argSearch(&cmd_args, "-serial") != -1;
    ...
    if (cmd_args.get(0) == SIGSTREN_CMD) {
      ...
      #endif
      wifi_ble_scan_obj.StartScan(WIFI_SCAN_SIG_STREN, TFT_MAGENTA);
    }
    // AP Scan
      ...
      if (full_sw == -1) {
        ...
        wifi_ble_scan_obj.StartScan(WIFI_SCAN_TARGET_AP_FULL, TFT_MAGENTA);
      }
    }
    // Raw sniff
    ...
    // Scan stations
      ...
      Serial.println("Starting Station scan. Stop with " + (String)STOPSCAN_CMD);  
        ...
        menu_function_obj.drawStatusBar();
      #endif
      wifi_ble_scan_obj.StartScan(WIFI_SCAN_STATION, TFT_ORANGE);
    }
    // Beacon sniff
    ...
    // Probe sniff
      ...
      Serial.println("Starting Probe sniff. Stop with " + (String)STOPSCAN_CMD);
        ...
        menu_function_obj.drawStatusBar();
      #endif
      wifi_ble_scan_obj.StartScan(WIFI_SCAN_PROBE, TFT_MAGENTA);
    }
    // Espressif sniff
      ...
      Serial.println("Starting Espressif device sniff. Stop with " + (String)STOPSCAN_CMD);
        ...
        menu_function_obj.drawStatusBar();
      #endif
      wifi_ble_scan_obj.StartScan(WIFI_SCAN_ESPRESSIF, TFT_MAGENTA);
    }

    //// Bluetooth scan commands
    // Bluetooth scan
    ...
    if (cmd_args.get(0) == UPDATE_CMD) {
      ...
      // Update via OTA
      //if (w_sw != -1) {
      //  Serial.println("Starting WaveSentry OTA Update. Stop with " + (String)STOPSCAN_CMD);
    ...
    // List Stations
      ...
      for (int x = 0; x < access_points->size(); x++) {
        ...
        for (int i = 0; i < access_points->get(x).stations->size(); i++) {
          wifi_ble_scan_obj.getMAC(sta_mac, stations->get(access_points->get(x).stations->get(i)).mac, 0);
  ...
  // Select access points or stations
    ...
    if (ap_sw != -1) {
      ...
      #endif
      wifi_ble_scan_obj.RunSaveAPList(true);
    }
  }
  ...
  // SSID stuff
    ...
    if (add_sw != -1) {
      ...
      if (gen_sw != -1) {
        ...
        wifi_ble_scan_obj.generateSSIDs(gen_count);
      }
      // Add specific
      else if (spc_sw != -1) {
        String essid = cmd_args.get(spc_sw + 1);
        wifi_ble_scan_obj.addSSID(essid);
      }
      else {
