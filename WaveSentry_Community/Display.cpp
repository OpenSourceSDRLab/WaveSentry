...
void Display::RunSetup()
{
  ...
  tft.setCursor(0, 0);
    ...
    #else if defined(TFT_DIY) // this is to set direction of startup
      uint16_t calData[5] = { 339, 3470, 237, 3438, 2 }; // parameter[4] is rotate or invert, now is default
      tft.setRotation(0); // Portrait with DIY TFT default

      //uint16_t calData[5] = { 339, 3470, 237, 3438, 4 }; 
      //tft.setRotation(2); // Portrait with DIY TFT inverted
      

      //uint16_t calData[5] = { 250, 3470, 237, 3700, 7 };  //fifth 7, tuned by s, for rotation 3   //second value is the position scale along the long edge //forth value is the position scale along the short edge
      //tft.setRotation(3); 
...
void Display::tftDrawSwitchButton() {
...
void Display::drawJpeg(const char *filename, int xpos, int ypos) {
  ...
  boolean decoded = JpegDec.decodeArray(WavesentryTitle, WAVESENTRY_TITLE_BYTES);
